...
struct Sim {
    ...
    : mutation_rate(m), ll_times(), ll_types(), sample_size(n), row_counts(),
    col_sums(), parent(2 * n - 1, -1), child_1(2 * n - 1, -1),
    child_2(2 * n - 1, -1), event_to_parent(n - 1, -1), node_times(2 * n - 1, 0),
    event_times(n - 1, 0)
    {
        gen = gsl_rng_alloc(gsl_rng_mt19937);
        gsl_rng_set(gen, time(NULL) * getpid());
    ...
    void sample_tree() {
        ...
        while (active.size() > 1) {
            ...
            time_increment = gsl_ran_exponential(gen, 1 / gsl_sf_choose(active.size(), 2));
            row = 0;
            ub = p[0];
            ...
            do {
                c2 = least_index + gsl_rng_uniform_int(gen, tmp_row_counts[row]);
            } while (c1 == c2);
    ...
    int number_of_mutations(const int i) const {
        ...
        if (i < sample_size) {
            for (unsigned int j = 0; j < col_sums.size(); j++) {
                if (gsl_matrix_get(data, sample_to_row(i), j) == 1 && col_sums[j] == 1) {
                    ret++;
                }
            }
        } else {
            ...
            if (parent[i] != -1) {
                ...
                for (unsigned int j = 0; j < col_sums.size(); j++) {
                    ...
                    for (unsigned int k = 0; k < leaves.size(); k++) {
                        leaf_sum += gsl_matrix_get(data, sample_to_row(leaves[k]), j);
                    }
                    if (leaf_sum == (int)leaves.size() && (int)leaves.size() == col_sums[j]) {
                        ret++;
                    }
                }
            }
    ...
    double log_likelihood_times() const {
        double ret = -exp(gsl_sf_lnchoose(sample_size, 2) + log(event_times[0]));
        for (int i = 1; i < sample_size - 1; i++) {
            ret -= exp(gsl_sf_lnchoose(sample_size - i, 2) + log(event_times[i] - event_times[i - 1]));
        }
        return ret;
    }
    ...
    double log_likelihood_types() const {
        ...
        for (int i = 0; i < 2 * sample_size - 1; i++) {
            if (parent[i] != -1) {
                ret += nmut[i] * (log(mutation_rate) + log(node_times[parent[i]] - node_times[i])) - mutation_rate * (node_times[parent[i]] - node_times[i]) - gsl_sf_lnfact(nmut[i]);
            }
        }
        return ret;
    }

    void detach_reattach(const int detach, const int attach_above, const double old_parent_time, const double new_parent_time) {
        ...
        event_to_parent.erase(event_to_parent.begin() + std::distance(event_times.begin(), std::lower_bound(event_times.begin(), event_times.end(), old_parent_time)));
        event_times.erase(std::lower_bound(event_times.begin(), event_times.end(), old_parent_time));
        event_to_parent.insert(event_to_parent.begin() + std::distance(event_times.begin(), std::upper_bound(event_times.begin(), event_times.end(), new_parent_time)), mother);
        event_times.insert(std::upper_bound(event_times.begin(), event_times.end(), new_parent_time), new_parent_time);
        return;
    }
    ...
    int mh_topology() {
        ...
        if (parent[attach_above] != -1 && child_1[detach] != -1 &&
            ...
            double new_parent_time = fmax(node_times[attach_above], node_times[detach]);
            double inc = 0;
            if (parent[attach_above] == -1) {
                inc = gsl_ran_exponential(gen, 1);
                log_alpha += inc;
            } else {
                inc = gsl_rng_uniform(gen) * (node_times[parent[attach_above]] - new_parent_time);
                log_alpha += log(node_times[parent[attach_above]] - new_parent_time);
            }
            new_parent_time += inc;
            if (parent[mother] == -1) {
                log_alpha -= node_times[mother] - fmax(node_times[sibling], node_times[detach]);
            } else {
                log_alpha -= log(node_times[parent[mother]] - fmax(node_times[sibling], node_times[detach]));
            }
            double old_ll_times = ll_times;
            double old_ll_types = ll_types;
            detach_reattach(detach, attach_above, old_parent_time, new_parent_time);
            ll_times = log_likelihood_times();
            ll_types = log_likelihood_types();
            log_alpha += ll_times + ll_types;
            if (log(gsl_rng_uniform_pos(gen)) > log_alpha) {
                ...
                detach_reattach(detach, sibling, new_parent_time, old_parent_time);
            }
        }
        return ret;
    }

    int mh_times(const double sd_numerator) {
        int ret = 1;
        ...
        for (int i = 0; i < sample_size - 1; i++) {
            sd = sd_numerator / sqrt((sample_size - 1) * (sample_size - i) * (sample_size - i - 1));
            lb = fmax(node_times[child_1[old_event_to_parent[i]]], node_times[child_2[old_event_to_parent[i]]]);
            mu = old_event_times[i];
            seed = gsl_rng_uniform(gen);
            seed += (1 - seed) * gsl_cdf_gaussian_P(lb - mu, sd);
            seed = mu + gsl_cdf_gaussian_Pinv(seed, sd);
            detach_reattach(child_1[old_event_to_parent[i]], child_2[old_event_to_parent[i]], mu, seed);
            log_alpha -= log(gsl_ran_gaussian_pdf(seed - mu, sd)) - log(1 - gsl_cdf_gaussian_P(lb - mu, sd));
        }
        for (int i = 0; i < sample_size - 1; i++) {
            sd = sd_numerator / sqrt((sample_size - 1) * (sample_size - i) * (sample_size - i - 1));
            lb = fmax(old_node_times[child_1[event_to_parent[i]]], old_node_times[child_2[event_to_parent[i]]]);
            mu = event_times[i];
            log_alpha += log(gsl_ran_gaussian_pdf(old_node_times[event_to_parent[i]] - mu, sd)) - log(1 - gsl_cdf_gaussian_P(lb - mu, sd));
        }
        double old_ll_times = ll_times;
        double old_ll_types = ll_types;
        ll_times = log_likelihood_times();
    ...
    int mh_mutation_rate(const double sd) {
        int ret = 1;
        double old_mutation_rate = mutation_rate;
        double log_alpha = -ll_types;
        mutation_rate += gsl_ran_gaussian_ziggurat(gen, sd);
    ...
    double tree_height() const {
        return event_times.back();
    }

    double branch_length() const {
        double ret = sample_size * event_times[0];
        for (int i = 1; i < sample_size - 1; i++) {
            ret += (sample_size - i) * (event_times[i] - event_times[i - 1]);
        }
        return ret;
    }
    ...
    int sample_size;
    std::vector<int> row_counts, col_sums, parent, child_1, child_2, event_to_parent, nmut;

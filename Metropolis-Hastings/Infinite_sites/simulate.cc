...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < run_length; i++) {
        accept_topology += sim.mh_topology();
        accept_times += sim.mh_times(sd_merger_times);
        accept_mutation += sim.mh_mutation_rate(sd_mutation_rate);
        std::cout << 2 * sim.mutation_rate << " " << sim.tree_height() << " "
            << sim.branch_length() << std::endl;
    }
    std::cout << (double)accept_mutation / run_length << " ";
    std::cout << (double)accept_times / run_length << " ";
    std::cout << (double)accept_topology / run_length << std::endl;
    return 1;
}

...
struct Sim {
    ...
    : mutation_rate(theta / 2), ll_times(), ll_types(), sample_size(n), nsites(m),
    row_counts(), parent(2 * n - 1, -1), child_1(2 * n - 1, -1), child_2(2 * n - 1, -1),
    event_to_parent(n - 1, -1), node_times(n - 1, 0.0), event_times(n - 1, 0.0), L()
    {
        gen = gsl_rng_alloc(gsl_rng_mt19937);
        gsl_rng_set(gen, time(NULL) * getpid());
    }
        ...
        while (n < sample_size + 1) {
            ...
            if (gsl_rng_uniform(gen) < theta / (n - 1 + theta)) {
                if (row_counts[child] > 1) {
                    ...
                    for (int i = 0; i < nsites; i++) {
                        // set to 1 first to make GSL resize the matrix properly even if the new entry is 0
                        gsl_spmatrix_set(tmp, (int)(row_counts.size() - 1), i, 1);
                        gsl_spmatrix_set(tmp, (int)(row_counts.size() - 1), i, gsl_spmatrix_get(tmp, child, i));
                    }
                    child = row_counts.size() - 1;
                }
                int site = gsl_rng_uniform_int(gen, nsites);
                gsl_spmatrix_set(tmp, child, site, ((int)gsl_spmatrix_get(tmp, child, site) + 1) % 2);
                int check;
                for (unsigned int i = 0; i < row_counts.size(); i++) {
                    ...
                    if ((int)i != child) {
                        ...
                        for (int j = 0; j < nsites; j++) {
                            if (gsl_spmatrix_get(tmp, i, j) != gsl_spmatrix_get(tmp, child, j)) {
                                check = 0;
                                break;
                            }
                        }
    ...
    void sample_tree() {
        ...
        for (int i = 0; i < (int)row_counts.size(); i++) {
            for (int j = 0; j < row_counts[i] - 1; j++) {
                sim_time += gsl_ran_exponential(gen, 1 / gsl_sf_choose(active.size(), 2));
    ...
    double log_mutation_transition(const double t, const int i, const int j) const {

...
struct Sim {
    ...
    : n(0), theta(), v_theta(), t_max(), sd_mut(), mh_rate(), row_count(), left_child(), right_child(), t(), v(), l(), L(), m(), M(), L_tmp(), M_tmp()
    {
    ...
    double log_likelihood_types() {
        ...
        for (int i = 0; i < nsites; i++) {
            ...
            ret += log(L_tmp[i][2 * n - 2][0] + L_tmp[i][2 * n - 2][1]) - log(2);
        }
        return ret;
    }
    ...
    int mh_theta() {
        ...
        theta += gsl_ran_gaussian(gen, sd_mut / nsites);
        if (theta < 0) {
            theta = -theta;
        }
    ...
    void run(double &accept_spr, double &accept_theta) {
        ...
        while (sim_time < t_max) {
            ...
            if (ind >= 0) {
                ...
                std::cout << theta * nsites << " " << tree_height() << " " << total_branch_length() << " 1" << std::endl;
                mh_steps++;
                ac_spr += spr();
                ac_theta += mh_theta();
            }
            std::cout << theta * nsites << " " << tree_height() << " " << total_branch_length() << " 0" << std::endl;
        }
    ...
    int n, nsites;
    double theta, v_theta, t_max, sd_mut, mh_rate;

...
struct Sim {
    ...
    Sim(std::string filename, const double mutation_v, const double hyb,
        ...
        : n(0), hybrid_rate(hyb), mutation_rate(), mutation_vel(mutation_v),
          hybrid_mutation_sd(mutation_sd), row_count(), col_sum(), left_child(),
          right_child(), nmut(), t(), v() {
        ...
        t.resize(n - 1, 0);
        nmut.resize(2 * n - 1, -1);
        mutation_rate = watterson_estimator();
        sample_tree();
    }
    ...
    // A helper method for generating an initial tree consistent with inifinite
    // sites data.
    ...
    // Returns the row of the SNP matrix to which the leaf of a given label
    // corresponds.
    ...
    // Stores the leaf labels subtended by a given node i.
    ...
    double log_likelihood_types() const {
        ...
        for (int i = 0; i < 2 * n - 2; i++) {
            ...
            if (nmut[i] > 0) {
                ret += nmut[i] * log(mutation_rate * bl / 2) -
                       mutation_rate * bl / 2 - gsl_sf_lnfact(nmut[i]);
            } else {
                ret -= mutation_rate * bl / 2;
            }
        }
        return ret;
    }
    ...
    double sample_increment(const int i, const double t_max,
        ...
        double rate = 0;
        if (i == n - 1) {
            rate = -(double)col_sum.size() /
                   (mutation_rate + mutation_vel * t_max);
            if (mutation_vel > 0) {
                for (int j = 0; j < n - 1; j++) {
            ...
            if (v[i] > 0) {
                rate = (n - i) *
                       (n + mutation_rate + fmax(mutation_vel * t_max, 0) - i -
                        1) /
                       2;
                for (unsigned int j = 0; j < branches.size(); j++) {
                    if (nmut[branches[j]] > 0 &&
                        branch_velocity(branches[j]) > 0) {
                        rate -= nmut[branches[j]] /
                                branch_length(branches[j], t_max);
                    } else if (nmut[branches[j]] > 0) {
                        rate -=
                            nmut[branches[j]] / branch_length(branches[j], 0);
                    }
                }
            } else {
                rate = (n - i) *
                       (n + mutation_rate + fmin(mutation_vel * t_max, 0) - i -
                        1) /
                       2;
                for (unsigned int j = 0; j < branches.size(); j++) {
                    if (nmut[branches[j]] > 0 &&
        ...
        do {
            ...
            if (ret >= t_max) {
                ...
                if (i == n - 1) {
                    alpha = -(double)col_sum.size() /
                            (mutation_rate + mutation_vel * ret);
                    for (int j = 0; j < n - 1; j++) {
                        alpha += (n - j) * (t[j] + v[j] * ret) / 2;
                    }
                    alpha *= mutation_vel / rate;
                } else {
                    alpha = (n - i) *
                            (n + mutation_rate + mutation_vel * ret - i - 1) /
                            2;
                    for (unsigned int j = 0; j < branches.size(); j++) {
                        if (nmut[branches[j]] > 0) {
                            alpha -= nmut[branches[j]] /
                                     branch_length(branches[j], ret);
                        }
                    }
                    alpha *= v[i] / rate;
                }
        ...
        while (sim_time < t_max) {
            ...
            for (int i = 0; i < n - 1; i++) {
                if (v[i] < 0) {
                    c = 1;
                    if (i == 0) {
                        if (nmut[left_child[0]] > 0 ||
                            nmut[right_child[0]] > 0) {
                            c = max_drop;
                        }
                    } else {
                        if ((n + i - 1 == left_child[i] ||
                             n + i - 1 == right_child[i]) &&
                            nmut[n + i - 1] > 0) {
                            c = max_drop;
                        }
            ...
            for (int i = 0; i < n; i++) {
                // i = 0, ..., n - 2 are the holding time increments. i = n - 1
